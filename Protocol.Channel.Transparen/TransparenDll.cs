...
namespace Protocol.Channel.Transparen
{
    ...
    public class TransparenDll : ITransparen
    {
        ...
        public TransparenDll() {
            onlineTimer= new System.Timers.Timer(60 * 1000);
        ...
        public bool DSStartService(ushort port)
        {
            try
            {
                ...
                InvokeMessage("开启接收器", "TCP");
            }
            catch
            {
                this.OnReceiverException();
                _stopReceiver = true;
                InvokeMessage("开启接收器失败", "TCP");
            }
            ...
            if (session != null && session.State == TSessionState.Normal)
            {
                lock (session)
                {
                    ...
                    catch
                    {
                        ...
                        InvokeMessage("发送下行指令", "TCP");
                    }
                }
                InvokeMessage("发送下行指令" + datagram, "发送");
            }
            return sendSuccess;
        }
        ...
        public bool SendData(uint sessionID, string datagram)
        {
            ...
            if (session != null && session.State == TSessionState.Normal)
            {
                ...
                InvokeMessage("回复上行接收确认标志" + datagram, "发送");
            }
            return sendSuccess;
        }
        ...
        private void ResolveBuffer(TSession session, int receivedSize)
        {
            try
            {
                ...
                {
                    session.bufferString = session.bufferString + messageStr;
                    return;
                }else
                {
                    if (session.bufferString != null && session.bufferString != "")
                    {
                        messageStr = session.bufferString + messageStr;
                        session.bufferString = "";
                    }
                }
                ...
                if (messageStr.Contains("$"))
                {
                    ...
                    result = messageStr.Substring(messageStr.IndexOf("$"));
                    string sid = result.Substring(messageStr.IndexOf("$") + 1, 10).Trim();

                    //根据站点ID绑定
                    foreach (TransparentHelper transparent in m_listTransparents)
                    {
                        if (transparent.stationId == sid)
                        {
                            transparent.ip = ip;
                            transparent.sessionId = sessionId.ToString();
                            break;
                                ...
                                result.Contains("1G25") || result.Contains("1G29") || result.Contains("+"))
                    {
                        ...
                        foreach (var msg in lists)
                        {
                            ...
                            if (ret && report != null)
                            {
                                ...
                                InvokeMessage("gprs号码:  " + sid + "   " + String.Format("{0,-10}   ", rtype) + plusMsg, "接收");
                                //TODO 重新定义事件
                ...
                if (messageStr.Contains("TRU"))
                {

                }
                if (messageStr.Contains("ATE0"))
                {
